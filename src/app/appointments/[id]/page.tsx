...
function displayUploadFilename(fullName: string) {
  ...
  // ✅ RIGHT-Pane Inhalt (Fotos/Doku). Wird auf Desktop rechts angezeigt und auf Mobile im Formular eingeblendet.
  const rightPane =
  isNew ? (
              <>
  ...
  const mobilePhotosBlock = (
    <div className="mobile-only" style={{ marginTop: 12 }}>
      ...
      >
        <div style={{ fontFamily: FONT_FAMILY, fontWeight: FW_SEMI, color: "#111827" }}>Fotos / Doku-Bilder</div>
        <button
          type="button"
          onClick={() => setMobilePhotosOpen((o) => !o)}
          style={{
            width: 36,
            height: 32,
            borderRadius: 12,
          ...
          aria-label={mobilePhotosOpen ? "Fotos einklappen" : "Fotos ausklappen"}
        >
          {mobilePhotosOpen ? "−" : "+"}
        </button>
      </div>

      {mobilePhotosOpen && <div style={{ marginTop: 10 }}>{rightPane}</div>}
    </div>
  );
...
return (
  ...
  />
      ...
      <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1.15fr 1fr", gap: 12, alignItems: "start" }}>
            ...
            {isNew ? (
              ...
              <div style={{ marginTop: 8, display: "grid", gap: 10 }}>
                <div style={{ padding: 12, borderRadius: 14, border: "1px solid #e5e7eb", background: "linear-gradient(#ffffff, #f9fafb)" }}>
                  ...
                  <div className="docHint" style={{ marginTop: 6, color: "#6b7280", fontFamily: FONT_FAMILY, fontWeight: FW_MED, fontSize: 12 }}>
        ...
        {/* RIGHT */}
        <section className="desktop-only" style={frameStyle}>
          {rightPane}
        </section>
      </div>
      </div>
